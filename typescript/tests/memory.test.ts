...
describe('Memory Leak Detection', () => {
  ...
  describe('Simple Event Memory Leaks', () => {
    it('should not leak memory with 10k simple events', async () => {
      ...
      if (memorySamples.length > 1) {
        ...
        expect(growthRate).toBeLessThan(15); // CI env + Node version variance
      }
    });
  ...
  describe('Mixed Signals Memory Usage', () => {
    it('should handle mixed signal types without memory leaks', async () => {
      ...
      // Memory growth should be reasonable (< 55 MB - Node.js memory management varies)
      // Increased threshold to account for GC timing variability
      expect(memoryGrowth).toBeLessThan(55);
    });
  });
