...
describe("AutomagikTelemetry", () => {
  ...
  describe("Constructor and Initialization", () => {
    ...
    it("should respect logs endpoint from environment variable", () => {
      ...
      process.env.AUTOMAGIK_TELEMETRY_LOGS_ENDPOINT =
        "https://loki.endpoint.com/v1/logs";
      const client = new AutomagikTelemetry(mockConfig);
      const status = client.getStatus();
      expect(status.endpoint).toBe("https://tempo.endpoint.com/v1/traces");
    ...
    it("config params should take precedence over environment variables for endpoints", () => {
      process.env.AUTOMAGIK_TELEMETRY_METRICS_ENDPOINT =
        "https://env-prometheus.endpoint.com/v1/metrics";
      process.env.AUTOMAGIK_TELEMETRY_LOGS_ENDPOINT =
        "https://env-loki.endpoint.com/v1/logs";
      const customConfig = {
        ...mockConfig,
      ...
      const status = client.getStatus();
      expect(status.metricsEndpoint).toBe(
        "https://config-prometheus.endpoint.com/v1/metrics",
      );
      expect(status.logsEndpoint).toBe(
        "https://config-loki.endpoint.com/v1/logs",
      );
      delete process.env.AUTOMAGIK_TELEMETRY_METRICS_ENDPOINT;
      delete process.env.AUTOMAGIK_TELEMETRY_LOGS_ENDPOINT;
    });
  ...
  describe("trackEvent", () => {
    ...
    it("should send OTLP-formatted payload", async () => {
      ...
      expect(payload.resourceSpans[0].scopeSpans[0].spans[0]).toHaveProperty(
        "name",
        "test.event",
      );
    });
    ...
    it("should truncate long string values", async () => {
      ...
      const longValueAttr = attributes.find(
        (attr: any) => attr.key === "long_value",
      );
    ...
    it("should handle different attribute types", async () => {
      ...
      const stringAttr = attributes.find(
        (attr: any) => attr.key === "string_value",
      );
      const numberAttr = attributes.find(
        (attr: any) => attr.key === "number_value",
      );
      const booleanAttr = attributes.find(
        (attr: any) => attr.key === "boolean_value",
      );
  ...
  describe("trackError", () => {
    it("should track error with message", async () => {
      ...
      const errorTypeAttr = attributes.find(
        (attr: any) => attr.key === "error_type",
      );
      const errorMessageAttr = attributes.find(
        (attr: any) => attr.key === "error_message",
      );
    ...
    it("should include error context", async () => {
      ...
      const errorCodeAttr = attributes.find(
        (attr: any) => attr.key === "error_code",
      );
      const operationAttr = attributes.find(
        (attr: any) => attr.key === "operation",
      );
    ...
    it("should truncate long error messages", async () => {
      ...
      expect(errorMessageAttr.value.stringValue.length).toBeLessThanOrEqual(
        500,
      );
    });
  });
  ...
  describe("trackMetric", () => {
    ...
    it("should include metric attributes", async () => {
      ...
      const operationTypeAttr = attributes.find(
        (attr: any) => attr.key === "operation_type",
      );
  ...
  describe("OTLP Metrics", () => {
    it("should send gauge metric", async () => {
      ...
      client.trackMetric("cpu.usage", 75.5, MetricType.GAUGE, {
        unit: "percent",
      });
      ...
      expect(payload.resourceMetrics[0].scopeMetrics[0].metrics[0].name).toBe(
        "cpu.usage",
      );
      expect(
        payload.resourceMetrics[0].scopeMetrics[0].metrics[0].gauge,
    ...
    it("should send counter metric", async () => {
      ...
      expect(
        payload.resourceMetrics[0].scopeMetrics[0].metrics[0].sum.isMonotonic,
      ).toBe(true);
    });
    ...
    it("should send histogram metric", async () => {
      ...
      client.trackMetric("request.duration", 123, MetricType.HISTOGRAM, {
        unit: "ms",
      });
  ...
  describe("OTLP Logs", () => {
    it("should send log with INFO severity", async () => {
      ...
      client.trackLog("Test log message", LogSeverity.INFO, {
        context: "test",
      });
    ...
    it("should send log with ERROR severity", async () => {
      ...
      client.trackLog("Error occurred", LogSeverity.ERROR, {
        error_code: "TEST-001",
      });
  ...
  describe("Edge Cases and Coverage", () => {
    ...
    it("should handle compression errors gracefully", async () => {
      ...
      expect(() =>
        client.trackEvent("test.event", { data: "test" }),
      ).not.toThrow();
      await new Promise((resolve) => setTimeout(resolve, 100));
    });
  ...
  describe("100% Coverage Tests", () => {
    ...
    describe("Compression error handling", () => {
      it("should handle gzip compression errors gracefully in OTLP backend", async () => {
        ...
        try {
          ...
          await expect(compressPayload("test")).rejects.toThrow(
            "Compression failed",
          );
        } finally {
          // Restore
    ...
    describe("Coverage completion tests", () => {
      it("should handle number values in system info attributes (line 322)", async () => {
        ...
        const cpuAttr = attributes.find(
          (attr: any) => attr.key === "system.cpu_count",
        );
      ...
      it("should handle verbose mode console.debug in sendMetric (lines 702-703)", async () => {
        ...
        let callCount = 0;
        const originalCreateAttributes = (client as any).createAttributes.bind(
          client,
        );
        jest
          .spyOn(client as any, "createAttributes")
    ...
    describe("Public API catch handlers", () => {
      it("should handle trackEvent catch handler (line 936)", async () => {
        ...
        jest
          .spyOn(client as any, "sendEvent")
          .mockRejectedValue(new Error("sendEvent error"));
      ...
      it("should handle trackMetric catch handler (line 992)", async () => {
        ...
        jest
          .spyOn(client as any, "sendMetric")
          .mockRejectedValue(new Error("sendMetric error"));
      ...
      it("should handle trackLog catch handler (line 1019)", async () => {
        ...
        jest
          .spyOn(client as any, "sendLog")
          .mockRejectedValue(new Error("sendLog error"));
      ...
      it("should handle flush timer catch handler (line 439)", async () => {
        ...
        const flushSpy = jest
          .spyOn(client as any, "flush")
          .mockRejectedValue(new Error("Forced flush error"));
